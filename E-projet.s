...
TexteDemandeNom:
    .asciiz "Veuillez entrer le nom de votre fichier: "
...
fichier:
    .space 1024 	# On réserve de la place pour le nom du fichier
...
# Affichage du menu
Menu:
    la $a0 TexteDemandeNom #On deamande le nom du fichier
    li $v0 4 #On dit que l'ont souhaite afficher une chaîne de caractère
    syscall #On effectue un appel système
    la $a0 fichier #On charge l'adresse de la chaine de caractère dans $a0
    li $a1 1024 #On réserve la place
    li $v0 8 #On lit la chaine
    syscall
    move $s4 $a0
    SupprRetChar:
    lw $a0 0($s4)
...
# Crée une pile qui stockera les indices des cases courantes
## Entrée : $a0 = taille du tableau
## 	    #a1 = indice case courante
##Sortie: $v0 = adresse du premier élément
Pile:
	# prologue
	subu $sp $sp 12
	sw $a0 8($sp)
